param(
    [string]$IndexPath = "demo-traces/cluster3-envelope-fault-matrix.index.json",
    [switch]$SkipBuild
)
...
if (-not $SkipBuild) {
    Write-Host "[cluster3-envelope-matrix] building runtime + tool..."
    cargo build -p runtime -p tool | Out-Host
}
else {
    Write-Host "[cluster3-envelope-matrix] build skipped (using existing binaries)"
}
...
$profiles = @(
    @{
        Name = "partition-dual12-isolation"
        EnvelopeType = "partition_like"
        PrimaryTarget = "node1"
        BootstrapBurst = 4
        NodeSteps = 340
        ReplaySteps = 460
        ...
        Node1Delay = 0; Node1Reorder = 1; Node1Drop = 1
        Node2Delay = 0; Node2Reorder = 1; Node2Drop = 1
        Node3Delay = 1; Node3Reorder = 2; Node3Drop = 0
    },
    @{
        Name = "partition-dual23-isolation"
        EnvelopeType = "partition_like"
        PrimaryTarget = "node2"
        BootstrapBurst = 4
        NodeSteps = 340
        ...
        MinClusterEvidence = 8
        MinActiveFaultNodes = 2
        Node1Delay = 2; Node1Reorder = 1; Node1Drop = 0
        Node2Delay = 0; Node2Reorder = 1; Node2Drop = 1
        Node3Delay = 0; Node3Reorder = 1; Node3Drop = 1
    },
    @{
        Name = "partition-tri-hard"
        EnvelopeType = "partition_like"
        PrimaryTarget = "node3"
        BootstrapBurst = 5
        NodeSteps = 360
        ...
        Node1Delay = 1; Node1Reorder = 2; Node1Drop = 3
        Node2Delay = 1; Node2Reorder = 2; Node2Drop = 3
        Node3Delay = 1; Node3Reorder = 2; Node3Drop = 3
    },
    @{
        Name = "churn-burst-light"
        EnvelopeType = "churn_like"
        PrimaryTarget = "node1"
        BootstrapBurst = 6
        NodeSteps = 380
        ReplaySteps = 520
        ...
        Node2Delay = 2; Node2Reorder = 2; Node2Drop = 0
        Node3Delay = 1; Node3Reorder = 2; Node3Drop = 0
    },
    @{
        Name = "churn-drop-mix"
        ...
        MinActiveFaultNodes = 3
        Node1Delay = 2; Node1Reorder = 3; Node1Drop = 5
        Node2Delay = 2; Node2Reorder = 2; Node2Drop = 4
        Node3Delay = 1; Node3Reorder = 3; Node3Drop = 5
    },
    @{
        Name = "churn-recovery-wave"
        EnvelopeType = "churn_like"
        PrimaryTarget = "node3"
        BootstrapBurst = 7
        NodeSteps = 420
        ...
        MinClusterEvidence = 16
        MinActiveFaultNodes = 3
        Node1Delay = 3; Node1Reorder = 2; Node1Drop = 3
        Node2Delay = 2; Node2Reorder = 3; Node2Drop = 0
        Node3Delay = 3; Node3Reorder = 2; Node3Drop = 3
    }
)
...
for ($i = 0; $i -lt $profiles.Count; $i++) {
    ...
    if ($activeNodes.Count -lt [int]$p.MinActiveFaultNodes) {
        throw ("profile {0} has only {1} active fault node(s), expected >= {2}" -f $p.Name, $activeNodes.Count, [int]$p.MinActiveFaultNodes)
    }

    $scenarioIndex = $i + 1
    $portBase = 8100 + ($scenarioIndex * 10)
    $idBase = 0xA00 + ($scenarioIndex * 10)
    $reportPrefix = "cluster3-envelope-$($p.Name)"
...
for ($i = 0; $i -lt $scenarios.Count; $i++) {
    ...
    Write-Host ("[cluster3-envelope-matrix] [{0}/{1}] running {2}" -f ($i + 1), $scenarios.Count, $s.Name)
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw ("cluster3 envelope scenario exited with code {0}" -f $LASTEXITCODE)
        }
    }
    catch {
        $scenarioError = $_.Exception.Message
        Write-Host ("[cluster3-envelope-matrix] scenario failed: {0}" -f $scenarioError)
    }
    ...
    $summary | Add-Member -NotePropertyName fault_envelope -NotePropertyValue $s.EnvelopeType -Force
...
foreach ($r in $results) {
    ...
    $envelope = [string]$r.fault_envelope
    if ($aggregateEnvelope.ContainsKey($envelope)) {
        $aggregateEnvelope[$envelope].scenario_count += 1
        if ($r.ok) {
            $aggregateEnvelope[$envelope].passed_count += 1
        }
        else {
            $aggregateEnvelope[$envelope].failed_count += 1
        }
    }
}
...
$index = [ordered]@{
    ...
    matrix = "cluster3-fault-envelope"
...
Write-Host ("[cluster3-envelope-matrix] index written: {0}" -f $IndexPath)
Write-Host ("[cluster3-envelope-matrix] scenarios passed: {0}/{1}" -f $passed.Count, $results.Count)

if ($failed.Count -gt 0) {
    $failedNames = @($failed | ForEach-Object { [string]$_.scenario })
    throw ("cluster3-envelope-matrix failed scenarios: {0}" -f ($failedNames -join ", "))
}
