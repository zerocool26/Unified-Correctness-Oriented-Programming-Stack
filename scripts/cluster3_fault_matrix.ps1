param(
    [string]$IndexPath = "demo-traces/cluster3-fault-matrix.index.json",
    [switch]$SkipBuild
)
...
if (-not $SkipBuild) {
    Write-Host "[cluster3-fault-matrix] building runtime + tool..."
    cargo build -p runtime -p tool | Out-Host
}
else {
    Write-Host "[cluster3-fault-matrix] build skipped (using existing binaries)"
}
...
foreach ($target in $targets) {
    foreach ($profile in $profiles) {
        $scenarioIndex += 1
        $portBase = 7800 + ($scenarioIndex * 10)
        $idBase = 0x800 + ($scenarioIndex * 10)
        $reportPrefix = "cluster3-fault-$target-$($profile.Name)"

        $scenarios += @{
            Name = "tri-$($profile.Name)-target-$target"
...
for ($i = 0; $i -lt $scenarios.Count; $i++) {
    ...
    Write-Host ("[cluster3-fault-matrix] [{0}/{1}] running {2}" -f ($i + 1), $scenarios.Count, $s.Name)
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw ("cluster3 fault scenario exited with code {0}" -f $LASTEXITCODE)
        }
    }
    catch {
        $scenarioError = $_.Exception.Message
        Write-Host ("[cluster3-fault-matrix] scenario failed: {0}" -f $scenarioError)
    }
...
foreach ($r in $passed) {
    $faultCountsTarget = if ($null -ne $r.fault_counts_target) { $r.fault_counts_target } else { $r.fault_counts_node2 }
    ...
    elseif ($null -ne $r.fault_counts_node2) {
        $aggregateFaultCountsByNode.node2.Drop += [int]$r.fault_counts_node2.Drop
        $aggregateFaultCountsByNode.node2.Delay += [int]$r.fault_counts_node2.Delay
        $aggregateFaultCountsByNode.node2.Reorder += [int]$r.fault_counts_node2.Reorder
    }
...
Write-Host ("[cluster3-fault-matrix] index written: {0}" -f $IndexPath)
Write-Host ("[cluster3-fault-matrix] scenarios passed: {0}/{1}" -f $passed.Count, $results.Count)

if ($failed.Count -gt 0) {
    $failedNames = @($failed | ForEach-Object { [string]$_.scenario })
    throw ("cluster3-fault-matrix failed scenarios: {0}" -f ($failedNames -join ", "))
}
